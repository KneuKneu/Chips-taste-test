...
function saveChipsResult() {
  ...
  console.log(`Current chip: ${currentChip} + ${parseInt(document.getElementById('flavor').value)}`);
  if (currentChip) {
...
chipName.innerHTML = 'Chip ' + chipsResult[0].name;
  ...
  chipsResult.forEach(chip => {
    ...
    button.addEventListener('click', () => {
      saveChipsResult();
      document.getElementById(`chip-${chip.name}`).className = "btn btn-primary m-1";
      ...
      chipsResult.forEach(otherChip => {
        if (otherChip.name !== chip.name) {
          document.getElementById(`chip-${otherChip.name}`).className = defaultButtonClass;
        }
      });
